...
export default function RecceFormScreen({ route, navigation }: RecceFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (reccePhotos.length === 0) {
      Alert.alert('Error', 'At least one recce photo is required');
      return;
    }

    for (let i = 0; i < reccePhotos.length; i++) {
      if (!reccePhotos[i].photo) {
        Alert.alert('Error', `Please capture photo for board ${i + 1}`);
        return;
      }
      if (!reccePhotos[i].width || !reccePhotos[i].height) {
        Alert.alert('Error', `Please enter measurements for board ${i + 1}`);
        return;
      }
    }
  ...
  const captureInitialPhoto = () => {
    if (initialPhotos.length >= 4) {
      Alert.alert('Limit Reached', 'Maximum 4 initial photos allowed');
      return;
    }
  ...
  const captureReccePhoto = (index: number) => {
    if (!reccePhotos[index].width || !reccePhotos[index].height) {
      Alert.alert('Measurements Required', 'Please enter width and height measurements before taking photos.');
      return;
    }
