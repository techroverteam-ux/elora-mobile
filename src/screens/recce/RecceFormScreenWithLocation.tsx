...
export default function RecceFormScreen({ route, navigation }: RecceFormProps) {
  ...
  const loadStoreData = async () => {
    try {
      const storeData = { address: '123 Main St, City, State 12345' };
      setFormData(prev => ({
        ...prev,
        originalAddress: storeData.address,
  ...
  const getCurrentLocationAndAddress = async () => {
    try {
      ...
      const storeImg = await locationService.getStoreImageFromMaps(location.latitude, location.longitude);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const recceData = {
        ...
        address: {
          original: formData.originalAddress,
          updated: formData.address,
        ...
        photos: {
          front: formData.photos.front ? {
            uri: formData.photos.front,
            measurements: `${formData.width}x${formData.height}ft`,
            capturedAt: new Date().toISOString()
          } : null,
          side: formData.photos.side ? {
            uri: formData.photos.side,
            measurements: `${formData.width}x${formData.height}ft`,
            capturedAt: new Date().toISOString()
          } : null,
          closeUp: formData.photos.closeUp ? {
            uri: formData.photos.closeUp,
            measurements: `${formData.width}x${formData.height}ft`,
            capturedAt: new Date().toISOString()
          } : null
        },
        submittedDate: new Date().toISOString(),
        measurementOverlayUsed: true
      };

      await storeAPI.submitRecce(storeId || recceId, recceData);
